...
/*공백이나 띄어쓰기는 관계없이 우너하는 SQL 구문만큼 드래그해서 실행*/
INSERT INTO kh_cafe (c_id, c_name, c_address, c_phone, c_time) VALUES
(2,'빠나쁘례쏘','서울시 강남구 역삼동','02-5555-5555','영업시간 12:00 ~ 14:00');
...
INSERT INTO kh_cafe (c_id, c_name, c_address, c_time) VALUES (5, '거피빈', '서울특별시 역삼동', '영업시간 평일 10:00 ~ 13:00');
...
-> 로그인, 게시판 글가져오기, 회원정보, 회사내 직원 정보, 급여내역, 스터디명단블로그 등
SELECT 어떤칼럼을볼지선택 FROM 어느테이블에서가져올지테이블명작성

한 테이블에 있는 모든 컬럼을 보겠다
SELECT * FROM  테이블이름;

한 테이블에 있는 특정 컬럼을 보겠다
SELECT C_NAME FROM  테이블이름;

한 테이블에 있는 특정된 컬럼들을 보겠다
