...
ORDER BY DEPT_CODE; 
-- ASC 기본값이기 때문에 오름차순 생략가능

/*
컬럼순서를 이용해서 정렬하는 방법*/
-- EMPLOYEE 테이블에서 급여가 300만원 이상, 600만원 이하인 사원의 사번 이름 급여를 이름 내림차순으로 조회
SELECT EMP_ID, EMP_NAME, SALARY FROM EMPLOYEE WHERE SALARY BETWEEN 3000000 AND 6000000 ORDER BY EMP_NAME DESC;

-- EMP_NAME : 현재 2번째 자리에 위치하는 순서를 이용해서 ORDER BY를 구할수 있음
SELECT EMP_ID, EMP_NAME, SALARY FROM EMPLOYEE WHERE SALARY BETWEEN 3000000 AND 6000000 ORDER BY 2 DESC;
-- 만약에 EMP_ID로 ORDER BY에서 정렬하길 원한다면 1을 작성
-- 만약에 SALARY로 ORDER BY에서 정렬하길 원한다면 3을 작성

-- ORDER BY 절에 수식적용
-- EMPLOYEE 테이블에서 이름, 연병을 연봉 내림차순 조회
...
-- SELECT절에서 해석된 별칭을 ORDER BY절에서 사용할 수 있음
...
-- ORDER BY는 결과를 나타내기 위해 찾는 조건문이기 때문
...
-- EMPLOYEE 테이블에서 이름, 부서코드, 급여를 
-- 부서코드 오름차순, 급여 내림차순 조회
-- 정렬해서 부서코드 어떻게 돼있고, 급여 가장 높은 금액만 궁금한것
...
D1 D2 D5 D6 D7 D8 D9 NULL 순서로 정렬
...
D1부서 직급코드 J7이 J6보다 먼저 출력되고, 
그 다음 J7은 단독이기 때문에 정렬할 수 있는거 없음
J6에서는 2명 이상이 존재하기 때문에 J6 직급의 직원들의 이름이 올므차순 정렬
...
FROM EMPLOYEE
ORDER BY 부서코드 ASC, 직급코드 DESC, 이름 ASC; // ORDER BY 부서코드, 직급코드 DESC, 이름;
