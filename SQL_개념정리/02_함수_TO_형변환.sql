...
SELECT 1234, TO_CHAR(1234) FROM DUAL; -- 테이블 타입 변경 숫자 -> 문자열
...
-- DAUL 테이블 사용해서 현재 날짜 2024-06-05(수)
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY-MM-DD(DY)')
FROM DUAL;

-- DAUL 테이블 사용해서 현재 날짜 2024-06-05(수요일)
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY-MM-DD(DAY)')
FROM DUAL;

-- DAUL 테이블 사용해서 현재 날짜 2024-06-05(수) 오전 시:분:초
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY-MM-DD(DY) AM HH:MI:SS')
FROM DUAL;

-- DAUL 테이블 사용해서 현재 날짜 2024년 06월 05일 오전 시:분:초
...
FROM DUAL; 
-- TO_DATE 형식으로 년/월/일 병경하겠다 06-05-2024 -> 24/05/06
...
-- 50 이상 : Y 현재 세기(2000 년대 기중 이후) R 이전 세기(1900년대 기준)
...
EXTRACT : 날짜뽑기
숫자관련 함수 CELL, ROUND, FLOOR, TRUNC ABS MOD
