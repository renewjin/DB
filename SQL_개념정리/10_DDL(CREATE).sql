...
---- [ 자료형 ]
            ...
            VACHAR2(크기) : 가변길이 문자형 (4000BYTE) VACHAR1의 업그레이드 버전
            
            ABC 문자열을 CHAR VARCHAR2 저장하면 어떻게 되는가?
            CHAR (10) 컬럼 : CHAR는 10을 모두 소진
            VACHAR2 (10) 컬럼 : VACHAR2 3만큼만 소진 나머지 7은 반환
...
-- 이름 : USER_NAME / 자료형 VARCHAR2(15)
-- 주민번호 : USER_SSN / 자료형 CHAR(14)
-- 가입일 : ENROLL_DATE / 자료형 DATE    - DEFAULT SYSDATE -- 현재시간을 기본값으로
...
--> DEFAULT 기본값을 지정하고 INSERT 나 UPDATE문을 작성해줄 때 지정한 기본값이 들어감
...
제약조건 종류
PRIMARY KEY, NOU NULL, UNIQUE, CHECK, FOREIGN KEY
...
CREATE TABLE UNIQUE_RABLE (
    ...
    -- 테이블 레벨 --
    --UNIQUE(USER_NAME) -- 컬럼 베벨 제약 조건 (이름X)
...
-- 한 테이블 당 한 개만 설정 가능
-- 컬럼 레벨, 테이블 베벨 둘 다 설정 가능
-- 한개 컬럼에 설정할 수 있고, 여러개의 컬럼을 묶어서 설정할 수 잇음
...
--> 기본키 중복으로 오류
--> USER_PRIMARY_TABLE 테이블에서 USER_NO 컬럼에 1 값이 존재하는데 추가로 1을 넣는다고 했기 때문에 중복으로 오류
*/

INSERT INTO USER_PRIMARY_TABLE VALUES (NULL, '이순신'); -- NULL 빈 칸 넣는다는 의미
-- 기본키가 NULL로 오류발생
...
-- UNIQUE는 컬럼에서 NULL이 한 개는 허용됨 두 개이상 안됨(어떤값이든 중복이 안됨)
-- PRIMARY KEY NULL값 안되고 중복안됨

/*
FORENIGN KEY(외부키) 제약조건
...
-- parent key not found 부모키 인 USER_GRADE에 40이라는 값이 없어서 넣을 수 없음
...
-- parent key not found 부모키 인 USER_GRADE에 50이라는 값이 없어서 넣을 수 없음
...
-- 배달어플 A카페 폐업
--- 배달 어플 -> A카페 카페 마음대로 폐업은 못함
---- A카페가 열어놓은 메뉴들은 모두 삭제하고 가
----- A카페 폐업할 때 연결된 메뉴까지 한번에 삭제
...
CREATE TABLE 자식테이블 (
    ...
    (방법2) - 부모키 삭제시 자식키를 NULL로 변경하는 옵셥 : ON DELETE SET NULL
    컬럼4 NUMBER CONSTRAINT 인덱스이름 REFERENCES 부모테이블(부모컬럼1) ON DELETE SET NULL
    
    (방법3) - 부모키 삭제시 자식키도 함께 삭제시킨다      : ON DELETE CASCADE       -> 가장많이 사용!!!!!
...
/*
ORA-02290: check constraint (KH_T.SYS_C007348) violated
제약조건으로 '남'이나 '여'만 가능하게 했는데 남자라는 조건이 들어와 에러발생
*/

INSERT INTO USER_CHECK VALUES (2, 'USER02', 'PW02', '박철수', '여');

INSERT INTO USER_CHECK VALUES (2, 'USER02', 'PW02', '강영희', '여자');
