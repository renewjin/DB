...
-- ALTER(바꾸다, 수정하다)
-- 수정가능한 것 컴럼(추가/수정/삭제) 제약조건(추가/삭제) 이름변경(테이블, 컬럼, 제약조건)
...
ALTER TABLE DEPT_COPY ADD CONSTRAINT DEPR_COPY_PK PRIMARY KEY (DEPT_ID);
--> ALTER TABLE DEPT_COPY : 테이블을 수정하겠다
--> ADD CONSTRAINT : 새로운 제약조건을 추가
--> DEPR_COPY_PK : 새로운 제약조건의 이름
...
-- 컬럼 추가 : ALTER TABLE 테이블명 ADD(컬럼명 데이터타입 [DEFAILT] '값']);
...
-->           ALTER TABLE 테이블명 DROP DROP COLUMN 삭제할컬럼명;
