...
[INCREAMENT BY 숫자] -- 다음 값에 대한 증갑값을 설정, 생략하면 자동으로 1씩 증가로 설정
[MAXVALUE 숫자 | NOMAXVALUE] -- 발생시킬 최대값 지정 EX) 최대 1000면 기록하겠다
[MINVALUE 숫자 | NOMINVALUE] -- 발생시킬 최소값 지정 EX) 21번부터 다시 시작하겠다.
[CYCLE | NOCYCLE] -- 값 순환 여부 지정
[CACHE 바이트크기 | NOCACHE] -- 캐쉬메모리 기본값은 20BYTE 최소값은 2BYTE
    --> 시퀀스의 캐시 메모리는 할당된 크기만큼 미리 다음 값을 생성해 지정해줌
    --> 시퀀스 호출 시 미리 저장되어진 값들을 가져와 반환하므로 매번 시퀀스를 생성해서 반환하는 것보다 DB속도향상
...
START WITH 100 -- 시작번호 100, 설정안해주면 1
INCREMENT BY 5 -- NEXTVAL 호출 마다 5씩 증가, 설정안해주면 1씩증가
...
-- 위 작성된 내용중 START 부터 NOCACHE까지 모두 생략가능
-- SQL에서 설정한 기본값이 자동으로 매겨짐

-- TB_TABLE 테이블 INSERT 시 시퀀스 사용하기
...
/*
-- 시퀀스 값 변경 (ALTER)
--> CREATE 구문과 똑같지만 START WITH 옵셥 제외됨
[작성법]
