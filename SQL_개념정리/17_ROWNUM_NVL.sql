...
--> ORA-00904: "DEPT": invalid identifier 오류해결 : DEPT 존재하지 않음
...
SELECT ROWNUM, EMP_ID, EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE ROWNUM <=5;
...
-- 1. SALARY 월급 높은 순서대로 출력한 다음 제대로 월급이 가장 높은 3명이 나온것이 맞는지 확인
...
NVL2 :
NULL이 아닌 컴럼의 값을 지정한 숫자 또는 문자로 변경해서 반환
[사용법]
NVL2(컬럼명, NULL이 아닌 경우 변경할 값, 컬럼값이 NULL일때 변경할 값)
*/
-- EMPLOYEE 테이블에서
-- 사번 이름 전화번호 조회
-- 전화번호가 없으면 없음으로 조회
