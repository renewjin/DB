...
--> 서브 쿼리를 통해서 특정 조건을 만족하는 최저급여를 찾고,
----> 조건에 맞는 급여를 받는 직원을 외부 커리에서 조회

/*
서브쿼리
SELECT MIN(SALARY)  --> 컬럼 최소값 선택
FROM EMPLOYEE       --> 찾을 테이터 테이블 선택
...
SELECT DEPT_TITLE, 
            /*최고 급여*/
(SELECT MAX(SALARY)
FROM EMPLOYEE E WHERE E.DEPT_CODE = D.DEPT_ID) AS "최고급여",
/*최저급여*/
...
-- 외부쿼리 DEPT_CODE
-- FROM DEPARTMENT D
SELECT DEPT_TITLE, (SELECT COUNT(*) FROM EMPLOYEE E WHERE E.DEPT_CODE = D.DEPT_ID) AS "직원수"
FROM department D;
